...
class ActivationScreen extends StatefulWidget {
  ...
  // 🔒 الكود مقسّم ومشفّر — لا يظهر كنص واضح في APK
  static const _p1 = 'aGF5dGhlbWdyb3VwQm';
  static const _p2 = 'RmKDE2MDYyMDAyKQ==';
  static String get _code {
    final bytes = base64.decode('$_p1$_p2');
...
class _ActivationScreenState extends State<ActivationScreen>
  ...
  Future<void> _validatePassword() async {
    ...
    if (entered != ActivationScreen._code) {
      _failedAttempts++;
      // ✅ SECURITY: تأخير متزايد بعد كل محاولة فاشلة (Anti-brute-force)
      if (_failedAttempts >= 3) {
        final delay = Duration(seconds: _failedAttempts * 2);
        setState(() => _error = 'كلمة السر غير صحيحة. انتظر ${delay.inSeconds} ثانية...');
    ...
    return Padding(
      ...
      child: Column(children: [
        ...
        if (kDebugMode) ...[
          ...
          TextButton.icon(
            onPressed: () => _onQrDetected(ActivationScreen._code),
            icon: const Icon(Icons.bug_report, size: 14, color: AppColors.textMuted),
            label: const Text(
              'تفعيل تجريبي — Debug فقط',
