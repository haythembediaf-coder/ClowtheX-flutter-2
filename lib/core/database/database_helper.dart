...
class DatabaseHelper {
  ...
  Future<Database> _initDatabase() async {
    ...
    return await openDatabase(
      path,
      version: 4,  // v4: performance indexes on created_at
      onCreate: _onCreate,
  ...
  Future<void> _onCreate(Database db, int version) async {
    ...
    ''');
    await db.execute('''
      CREATE INDEX idx_sales_created_at ON ${AppConstants.tableSales}(created_at)
    ''');
    await db.execute('''
      CREATE INDEX idx_sales_status ON ${AppConstants.tableSales}(status)
    ''');
    await db.execute('''
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    // Migration v3 -> v4: Add performance indexes
    if (oldVersion < 4) {
      final indexes = [
        'CREATE INDEX IF NOT EXISTS idx_sales_created_at ON ${AppConstants.tableSales}(created_at)',
        'CREATE INDEX IF NOT EXISTS idx_sales_status ON ${AppConstants.tableSales}(status)',
        'CREATE INDEX IF NOT EXISTS idx_expenses_date ON ${AppConstants.tableExpenses}(date)',
      ];
      for (final sql in indexes) {
        try { await db.execute(sql); } catch (e) { debugPrint('Index migration note: \$e'); }
      }
    }
  }
