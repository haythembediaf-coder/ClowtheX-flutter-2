...
class ProductService {
  ...
  /// ✅ FIX: تنظيف صور المنتجات اليتيمة (غير المرتبطة بأي منتج)
  /// استدعِها من صفحة الإعدادات أو عند الـ backup
  Future<int> cleanOrphanImages() async {
    ...
    try {
      ...
      // اجمع كل مسارات الصور المسجّلة في قاعدة البيانات
      final rows = await _db.rawQuery(
        'SELECT image_path FROM ${AppConstants.tableProducts} WHERE image_path IS NOT NULL AND image_path != \'\'',
      );
      final registeredPaths = rows
          .map((r) => r['image_path'] as String?)
          .where((path) => path != null && path.isNotEmpty)
          .toSet();

      // احذف كل ملف في المجلد غير موجود في القاعدة
