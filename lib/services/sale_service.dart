...
abstract class SaleServiceBase {
  ...
  Future<List<Sale>> getAllSales({int limit = 500, int offset = 0});
...
class SaleService implements SaleServiceBase {
  ...
  Future<void> _initCounter() async {
    ...
    try {
      ...
      final result = await _db.rawQuery(
        "SELECT MAX(CAST(SUBSTR(invoice_number, 10) AS INTEGER)) as max_num "
        "FROM \${AppConstants.tableSales} WHERE invoice_number LIKE 'INV-\$year-%'",
      );
  ...
  Future<List<Sale>> getAllSales({int limit = 500, int offset = 0}) async {
